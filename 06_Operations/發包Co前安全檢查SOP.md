...
## 無人看守派工後的亂碼檢查（2026-08-31新增）

**背景**：妹的桌機只有她一人使用，作息是深夜才睡、睡前會直接關機（不是睡眠模式），所以「無人看守」的派工視窗其實是「派工後到她關機前」這段時間，不是真的整晚——電腦真的關機後，任何本機排程（Codex自動化、CC自己的scheduled-tasks）都不會執行，也不會在開機時自動補跑。

**風險**：無人看守時如果真的撞上平行執行亂碼bug（見上方），沒有人能在當下發現、即時清掉，可能整批檔案寫壞了要等隔天才發現。

**因應規則**：
1. **無人看守的派工，一次只派一個任務**，不要為了效率同時排好幾個（降低撞上平行執行的機率，即使發生了影響範圍也只有一個任務）
2. **任務完成後、妹隔天檢查之前，先自動跑一次亂碼掃描**，用git diff掃過所有被改動的檔案，抓已知的亂碼特徵：
...
if ($changed) {
  ...
  foreach ($f in $changed) {
    if (Test-Path $f) {
      ...
      if ($content -match '\?{4,}' -or ($content -and $content.Contains([char]0xFFFD))) {
        $bad += $f
      }
    }
  }
  if ($bad.Count -gt 0) {
    Write-Output "⚠️ 疑似亂碼，以下檔案需要人工確認："
    $bad | ForEach-Object { Write-Output " - $_" }
  } else {
    Write-Output "✅ 已掃描 $($changed.Count) 個變更檔案，沒有發現已知亂碼特徵（連續問號／Unicode替換字元）"
  }
} else {
  Write-Output "沒有偵測到git變更檔案"
}
```

3. **這個掃描只抓得到「已知的亂碼特徵」（連續問號、Unicode替換字元），不是內容品質檢查**——掃描過關不代表內容正確，只代表沒有撞上這個特定的技術性故障。內容本身還是要照原本的流程驗收（不信自報，實際打開看）。
4. 掃描結果如果抓到疑似亂碼的檔案，**先不要自動復原或刪除**，保留現狀讓妹/CC決定怎麼處理，避免在還沒確認問題範圍之前又做了新的動作。
