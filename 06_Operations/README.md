...
## 內容製作方法論（跨格式通用，任何新內容任務動工前先看）
| SOP | 位置 | 用途 |
|---|---|---|
| **內容製作方法論.md** | [06_Operations/內容製作方法論.md](內容製作方法論.md) | 遇到下面表格沒收錄的新內容形式（動畫、漫畫、角色影片、唱歌、直播、互動角色等）時，**先套用這份原則**：以「目標成品／效果」反向搜尋全球現有捷徑（App/外掛/模型/LoRA/ComfyUI/開源專案/API/自動化等），評估後再決定用現成捷徑還是自組工作流——不要先從已知或已訂閱的工具開始做。2026-09-23建立 |
...
| SOP | 位置 | 用途 |
|---|---|---|
| 可翻閱電子繪本除錯SOP | `有的沒的小舖\推廣\心理學\讀心日記\暗黑心理學\操控與控制\_可翻閱電子繪本除錯SOP_lulu_20260803.md` | 互動翻頁繪本 index.html 的測試環境、JS bug、CSS、CTA 頁除錯方法 |
| 繪本影片內容結構SOP | `有的沒的小舖\推廣\心理學\讀心日記\暗黑心理學\操控與控制\_繪本影片內容結構SOP_lulu_20260812.md` | 文字/故事結構層面製作流程（跟上面除錯SOP不同層次）：大綱先行+改完要驗證是否真的解決根本問題，含結構規則清單跟GPT/CC/Jane三方分工，適用暗夜觀察日記全系列13冊 |

## 影片製作
| SOP | 位置 | 用途 |
|---|---|---|
| Ken Burns運鏡推廣影片SOP | `有的沒的小舖\推廣\心理學\讀心日記\暗黑心理學\操控與控制\_KenBurns運鏡推廣影片SOP_lulu_20260803.md` | SG/Grok Imagine 生成運鏡片段→下載→TTS→字幕配樂組裝全流程 |
...
| 魯魯觀點素材庫使用說明 | `有的沒的小舖\推廣\平台\01_Content_Master\心理學\魯魯觀點素材庫\README.md` | 心理學書院28本書擷取觀點/金句的方法SOP＋魯魯口吻轉換規則＋Excel索引欄位說明（`sync_index.py`同步腳本，桌面`魯魯觀點素材庫_使用追蹤.xlsx`為使用者工作副本） |
...
| 文件 | 位置 | 用途 |
|---|---|---|
| CC_CC2_Relay.md | [06_Operations/CC_CC2_Relay.md](CC_CC2_Relay.md) | CC×CC2直接交辦任務的共用待辦檔（待處理／已完成兩區塊），比照ST repo `docs/CC2_task_queue.md`慣例，減少每次靠妹人肉橋接；由2026-09-19設定的每小時Routine自動讀取處理，不取代妹/小策即時轉話管道 |
